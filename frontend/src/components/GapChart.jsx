  ...
  return (
    <>
      <div className="legend">
        <span>
          <i className="swatch" style={{ background: 'var(--diverge-over)' }} /> More than {benchmarkLabel}
        </span>
        <span>
          <i className="swatch" style={{ background: 'var(--diverge-under)' }} /> Less than {benchmarkLabel}
        </span>
      </div>
      ...
      <div className="chart-scroll">
        <svg
          ...
          role="img"
          aria-label={`Difference in allocation versus ${benchmarkLabel}, in percentage points, by asset class`}
        >
          {[-max, -max / 2, 0, max / 2, max].map((t, i) => (
            <g className="tick" key={i}>
              <line x1={mid + x(t)} x2={mid + x(t)} y1={PAD_T} y2={PAD_T + rows.length * ROW_H} />
              <text x={mid + x(t)} y={height - 8} textAnchor="middle">
                {t > 0 ? '+' : ''}
                {t.toFixed(0)}pp
              </text>
            </g>
          ...
          {rows.map((row, i) => {
            ...
            const tipContent = (
              <TooltipRows
                title={row.label}
                rows={[
                  { label: 'You', value: pct(row.user_pct / 100) },
                  { label: benchmarkLabel, value: pct(row.benchmark_pct / 100) },
                  { label: 'Difference', value: pp(row.gap_pp) },
                ]}
              />
            )
            return (
