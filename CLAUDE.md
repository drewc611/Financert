...
the files it lints are declared in the config rather than passed as `--ext`.
...
## Localisation and the installable app

`frontend/src/i18n/` is a hand-rolled translation layer -- ~90 lines, no
dependency, because `Intl` already does the hard part. Six locales ship:
English, Spanish, French, German, Portuguese and Arabic. Adding a seventh is a
data change: drop a JSON file in `locales/` and add one row to `LOCALES`.

Four things here are load-bearing:

1. **Asset-class and tier names are translated too**, keyed by the backend's
   stable identifiers (`corporate_equities`, `top1`). Those labels arrive from
   the API in English; translating only the chrome would leave the actual
   content of the page in English. Same for `percentiles.*` -- the API sends
   English ordinals ("99th-100th") that don't survive translation.
2. **Never call `lib/format` directly from a component.** Use `fmt` from
   `useI18n()`, which is pre-bound to the active locale. A direct import
   formats German numbers in en-US and nothing catches it.
3. **Chart label gutters are measured, not fixed** (`lib/chartLabels.js`). A
   gutter sized for English clips translated labels -- 30-50% text expansion is
   normal, and "Participations dans des entreprises privées" was cut to "ns des
   entreprises privées" before this existed.
4. **Don't name a variable `t` inside a component.** It shadows the translation
   function. Three chart tick callbacks did exactly that; they use `tick` now.

Non-English locales carry `disclaimer.translationNote`, which says the English
original governs. These translations have not been through legal review and the
"not investment advice" paragraph is the one that matters -- keep that note
until they have been.

RTL works by `dir` on `<html>`, which flips the flexbox layout for free. Two
exceptions are pinned LTR in `styles.css`: the SVG charts, whose geometry is
computed in user space that `dir` does not mirror, and Latin-script inputs
(slugs, tokens) whose punctuation reorders otherwise. Mirroring the chart
geometry properly is unbuilt.

### PWA

`public/manifest.webmanifest`, `public/sw.js` and `public/icons/` (generated by
`tools/make_icons.py`, stdlib only) make the dashboard installable and usable
offline. `main.jsx` registers the worker **in production builds only** -- in dev
it would serve cached bundles over Vite's HMR, which looks exactly like edits
not taking effect.

The service worker precaches no CSS or JS on purpose: Vite fingerprints them,
so `/styles.css` ships as `/assets/index-<hash>.css` and naming it in the
precache list caches a 404. Navigations are network-first (a stale index.html
would pin the app to an old bundle); hashed assets are cache-first (the hash
makes them immutable); `/api/*` is never cached, because a silently stale
portfolio is worse than the visible "Offline snapshot" badge the app already
has.
